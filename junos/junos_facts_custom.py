...
class Interfaces(FactsBase):
    ...
    def populate(self):
        ...
        for item in reply[0]:
            ...
            mac = self.get_text(item, 'hardware-physical-address')
            obj = {
            ...
            logint = item.findall('logical-interface')
            for log in logint:
                name = self.get_text(log, 'name')
                obj = {
                    'description': self.get_text(log, 'description'),
                    'encapsulation': self.get_text(log, 'encapsulation'),
                }
                try:
                    addr = str(log.find('address-family').find('interface-address').find('ifa-local').text).strip()
                    net = str(log.find('address-family').find('interface-address').find('ifa-destination').text).strip()
                    if net == 'Unspecified':
                        masklen = '32'
                    else:
                        ip = IPNetwork(net)
                        masklen = ip.prefixlen
                    if addr != '127.0.0.1':
                        if str(log.find('address-family').find('address-family-name').text).strip() == 'inet':
                            self.facts['all_ipv4_addresses'].append(addr)
                            obj.update({'ipv4': [{'address': addr, 'masklen': masklen}]})
                        if str(log.find('address-family').find('address-family-name').text).strip() == 'inet6':
                            self.facts['all_ipv6_addresses'].append(addr)
                            obj.update({'ipv6': [{'address': addr, 'masklen': masklen}]})
                except AttributeError:
                    addr = ''
                    masklen = ''
                try:
                    mtu = str(log.find('address-family').find('mtu').text).strip()
                    obj.update({'mtu': mtu})
                except AttributeError:
                    mtu = ''
                try:
                    vlan = str(log.find('link-address').text)
                    obj.update({'vlan': vlan.split('.')[1][:-2].strip()})
                except AttributeError:
                    vlan = ''
                if name.find('local') == -1:
                    interfaces[name] = obj
        
        rele = Element('get-instance-information')
        SubElement(rele, 'detail')
        instances = exec_rpc(self.module, tostring(rele))
        
        
        for instance in instances[0]:
            vrfname = self.get_text(instance, 'instance-name')
            
            rint = instance.findall('instance-interface')
            for r in rint:
                intname = self.get_text(r, 'interface-name')
                interfaces[intname].update({'vrf': vrfname})
